...
<template>
	<nav class="category-nav">
		<ul class="category-menu">
			...
			<li class="category-item" v-for="category in categories" :key="category.id" link @click="selectCategory(category.id)">
				<a href="#" :class="{ 'active': $route.name === 'category' && Number($route.params.categoryId) === category.id }">{{ category.name }}</a>
			</li>
		</ul>
	</nav>
